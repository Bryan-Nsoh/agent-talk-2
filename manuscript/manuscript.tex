...
Agents interact with the environment via a structured JSON prompt-response loop. They do not receive visual input; instead, they receive a local sensor patch and a persistent memory object (an ASCII map). Figure \ref{fig:schema} illustrates the input-output contract. The input prompt aggregates the agent's immediate surroundings (\texttt{local\_patch}) and any messages received from the previous turn (\texttt{inbox}). In the Communication conditions, the agent's output includes an explicit \texttt{message} payload that is broadcast to teammates. In the Shared Representation conditions, this message channel is disabled; instead, the private \texttt{world\_map\_ascii} shown in the prompt is automatically synchronized with other agents by the environment.
...
Our results indicate that in bandwidth-constrained environments, shared representations are a potent mechanism for multi-agent search, offering immediate performance gains over independent exploration. However, they do not inherently solve coordination; by aligning agent priors, they can exacerbate contention in bottlenecks.

Explicit communication, while theoretically capable of resolving this contention, did not yield statistically significant performance improvements in our setup. Our qualitative analysis suggests that explicit messaging imposes inherent synchronization costs: agents often pause to broadcast and await confirmation, consuming turns that might otherwise be used for movement. The realistic 1-turn latency ($T+1$) exacerbates this friction by forcing conservative stop-and-wait behaviors that neutralize the efficiency gains of coordination.
