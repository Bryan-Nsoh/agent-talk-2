...
In the Shared Representation branch, the \texttt{goal\_sensor} is removed entirely. Agents must explore the grid blindly until they visually discover the goal symbol. The message channel is also disabled; instead, the internal map state is automatically synchronized with other agents (either upon contact in the Radio condition, or globally in the Global condition). This design tests whether shared environmental knowledge can substitute for both goal guidance and explicit messaging. Figure~\ref{fig:grid} shows the environment during a typical run, while Figure~\ref{fig:interface} illustrates the fog-of-war partial observability.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\columnwidth]{figures/fog_of_war_example.png}
    \caption{Partial Observability (Fog of War). Agents perceive only their immediate neighborhood (visibility radius = 1). Gray cells represent unknown territory.}
    \label{fig:interface}
\end{figure}
...
We group the six conditions into two investigation branches, summarized in Table~\ref{tab:design}. Each branch utilizes a specific control baseline matching its interface (Sensor vs. ASCII Map) to ensure fair comparison.
...
\toprule
\textbf{Paradigm} & \textbf{Condition} & \textbf{Map} & \textbf{Comm.} & \textbf{Protocol} & \textbf{Hypothesis} \\
\midrule
...
\centering
\small
\caption{Statistical Summary (Mann-Whitney U)}
\label{tab:stats}
\begin{tabular}{llll}
\toprule
\textbf{Comparison} & \textbf{Metric} & \textbf{p} & \textbf{Sig.} \\
\midrule
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\columnwidth]{figures/mapshare_agent_success_vs_baseline.png}
